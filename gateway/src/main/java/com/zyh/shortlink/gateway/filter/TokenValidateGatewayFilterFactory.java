package com.zyh.shortlink.gateway.filter;

import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONObject;
import com.fasterxml.jackson.annotation.JsonFormat;
import com.zyh.shortlink.gateway.config.Config;
import com.zyh.shortlink.gateway.dto.GatewayErrorResult;
...
public class TokenValidateGatewayFilterFactory extends AbstractGatewayFilterFactory<Config> {
        ...
        return (exchange,chain)->{
            ...
            if(!isPathInWhiteList(requestPath,requestMethod,config.getWhitePathList())){
                ...
                if(StringUtils.hasText(username)&&StringUtils.hasText(token)&&(userInfo=stringRedisTemplate.opsForHash().get("short-link:login:"+username,token))!=null){
                    ...
                    ServerHttpRequest.Builder headers = exchange.getRequest().mutate().headers(httpHeaders -> {
    ...
    private Boolean isPathInWhiteList(String requestPath, String requestMethod, List<String> whitePathList){
        return (!CollectionUtils.isEmpty(whitePathList)&&whitePathList.stream().anyMatch(requestPath::startsWith))||(Objects.equals(requestMethod,"Post")&&Objects.equals(requestPath,"/api/short-link/admin/v1/user"));
    }
}
